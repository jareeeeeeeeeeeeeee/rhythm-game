...
func _input(ev):
	...
	if Input.is_action_just_pressed("button4") and not ev.echo and get_tree().get_nodes_in_group("3"):
		...
		lane4.modulate.a = 1
	if Input.is_action_just_pressed("button5") and not ev.echo and get_tree().get_nodes_in_group("4"):
		var hit_note = get_tree().get_nodes_in_group("4")[0]
		print(hit_note.position.y)
		hit_note.queue_free()
	elif Input.is_action_just_pressed("button5") and not ev.echo and not get_tree().get_nodes_in_group("4"):
		lane5.modulate.a = 1
	if Input.is_action_just_pressed("button6") and not ev.echo and get_tree().get_nodes_in_group("5"):
		var hit_note = get_tree().get_nodes_in_group("5")[0]
		print(hit_note.position.y)
		hit_note.queue_free()
	elif Input.is_action_just_pressed("button6") and not ev.echo and not get_tree().get_nodes_in_group("5"):
		lane6.modulate.a = 1
	if Input.is_action_just_pressed("button7") and not ev.echo and get_tree().get_nodes_in_group("6"):
		var hit_note = get_tree().get_nodes_in_group("6")[0]
		print(hit_note.position.y)
		hit_note.queue_free()
	elif Input.is_action_just_pressed("button7") and not ev.echo and not get_tree().get_nodes_in_group("6"):
		lane7.modulate.a = 1
	if Input.is_action_just_pressed("button8") and not ev.echo and get_tree().get_nodes_in_group("7"):
		var hit_note = get_tree().get_nodes_in_group("7")[0]
		print(hit_note.position.y)
		hit_note.queue_free()
	elif Input.is_action_just_pressed("button8") and not ev.echo and not get_tree().get_nodes_in_group("7"):
		lane8.modulate.a = 1
